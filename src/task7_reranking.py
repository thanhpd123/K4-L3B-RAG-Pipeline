...
def rerank_rrf(
    ...
    for ranked_list in ranked_lists:
        # A malformed input list must not give one document multiple votes.
        ...
        for rank, item in enumerate(ranked_list, start=1):
            ...
            if item_id not in items:
                items[item_id] = item
                encounter_order[item_id] = len(encounter_order)

    ranked_ids = sorted(
        scores,
        key=lambda item_id: (-scores[item_id], encounter_order[item_id]),
    )
    results = []
